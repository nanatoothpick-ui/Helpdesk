...
The current frontend already has the provided Web App URL configured in `app.js`.

If you ever need to replace the backend URL, log in first, then go to:

```txt
Settings → Advanced → Google Sheets Backend
```

Expand the hidden backend panel, paste the new URL, and click:
