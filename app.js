...
const loginBackendUrl = document.getElementById("loginBackendUrl");
...
function updateBackendStatus() {
  if (loginBackendUrl) loginBackendUrl.value = backendUrl;
...
loginForm.addEventListener("submit", async (event) => {
  ...
  setBackendUrl(loginBackendUrl.value);

  if (backendEnabled()) {
    try {
      loginError.textContent = "Connecting to Google Sheets...";
      ...
      loginError.textContent = "";
      showApp();
      showToast(`Logged in as ${activeUser} using Google Sheets.`);
    } catch (error) {
  ...
  sessionStorage.setItem(STORAGE_KEYS.session, activeUser);
  showApp();
  showToast(`Logged in as ${activeUser} in local mode.`);
});
...
sampleMemoBtn.addEventListener("click", () => {
  ...
  updateMemoPreview();
  scrollToElement("memoSection");
  showToast("Sample memo loaded. You can edit and save it.");
});
