...
App.auth = (function () {
  ...
  /* ----------------------------------------------------------------------
     SIGN UP
     Creates a brand new account on the server and logs the person straight
     in, the same way login() does. There is no admin approval step — this
     matches the old app's model of "anyone with the URL and a password is
     trusted", just with each person choosing their own password now
     instead of starting from a published default.
     ---------------------------------------------------------------------- */
  async function signup(name, password, confirm) {
    if (store.isLocalMode()) {
      throw new Error("This app is set to work on this device only, so accounts are not used. Just type your name and continue.");
    }
    if (!name)     throw new Error("Please enter your name.");
    if (!password) throw new Error("Please choose a password.");
    if (password.length < App.config.minPasswordLength) {
      throw new Error("Your password must be at least " + App.config.minPasswordLength + " characters long.");
    }
    if (password !== confirm) throw new Error("The two passwords do not match.");

    let result;
    try {
      result = await api.requestWithRetry("signup", {
        name: name, password: password, confirmPassword: confirm
      });
    } catch (err) {
      if (err.message === "OFFLINE")   throw new Error("Cannot reach the server. Check your internet connection.");
      if (err.message === "TIMEOUT")   throw new Error("The server took too long to reply. Please try again.");
    ...
    if (!result.token) {
      throw new Error("The server did not confirm the new account. Please try logging in with the details you just chose.");
    }
    store.saveSession(result.user || name, result.token);
    return { user: result.user || name, data: result.data || null };
  }
  ...
  return { login, logout, signup, changePassword, restore, sessionExpired };
...
App.boot = (function () {
  ...
  /* Switch between the login form and the "create an account" form. Local
     mode never shows signup at all (see applyModeToLoginForm), so this only
     really matters once a backend is connected. */
  function showLoginView(view) {
    ...
    if (isSignup) { $("signupName").focus(); } else { $("loginTechnician").focus(); }
  }

  /* In local mode there is no server to check a password, so rather than
     pretending, we hide the password box and say so plainly. Accounts (and
     therefore "create an account") only make sense once a server is
     connected, so the signup link is hidden in local mode too. */
  function applyModeToLoginForm() {
    const local = store.isLocalMode();
    const passwordField = $("loginPassword").closest(".field");
    const button = $("loginButton");
    const hint = document.querySelector(".login-hint");
    const signupWrap = $("signupToggleWrap");
    ...
    if (local) {
      ...
      if (signupWrap) signupWrap.hidden = false;
    }
    showLoginView("login");
  }
  ...
  async function handleSignup(e) {
    ...
    try {
      const result = await auth.signup(name, password, confirm);
      const data = result.data || await api.getAll();
      store.setAll(data);
      ui.showApp();
      ui.navigate("dashboard");
      ui.toast("Welcome, " + (result.user || name) + ". Your account is ready.", "ok");
    } catch (err) {
      errorBox.textContent = err.message;
      errorBox.scrollIntoView({ behavior: "smooth", block: "nearest" });
    } finally {
  ...
  async function start() {
    ...
    $("signupToggle").addEventListener("click", () => showLoginView("signup"));
    $("signupBackToLogin").addEventListener("click", () => showLoginView("login"));
    bindPasswordToggles();
