  ...
  async function syncOutbox() {
    ...
    for (const entry of pending) {
      ...
      try {
        ...
        if (isTransient(err)) {
          /* Still offline. Stop here and keep everything queued. */
...
App.auth = (function () {
  ...
  async function login(technician, password) {
    ...
    if (!result.token) {
      throw new Error(
        ...
        "Please update it: see step 4 of the README (\"Update your Google Sheet backend\")."
      );
    }
  ...
  function sessionExpired() {
    store.wipeOnLogout();
    App.ui.toast("Your session expired. Please log in again.", "warn");
    App.ui.showLogin();
  }
...
App.settings = (function () {
  ...
  function saveUrl() {
    ...
    ui.toast(url ? "Backend URL saved. Please log in again." : "Switched to this device only.", "ok");
