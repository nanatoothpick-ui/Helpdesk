...
const quickReportsButton = document.querySelector("[data-open-reports]");
...
const exportSystemReportBtn = document.getElementById("exportSystemReportBtn");
const printSystemReportBtn = document.getElementById("printSystemReportBtn");
const reportTotalCount = document.getElementById("reportTotalCount");
const reportIncidentCount = document.getElementById("reportIncidentCount");
const reportMemoCount = document.getElementById("reportMemoCount");
const reportInventoryCount = document.getElementById("reportInventoryCount");
const systemReportOutput = document.getElementById("systemReportOutput");
...
function pageForElement(id) {
  ...
  if (id === "reportsSection") return "reports";
...
function showPage(page = "dashboard", shouldScroll = true) {
  ...
  const pageText = {
    ...
    reports: ["Incident Reports", "Load weekly, monthly, custom, or all-time incident reports."],
...
function getReportRange() {
  ...
  if (period === "thisWeek") {
    ...
    from = reportFromDate.value ? startOfDay(new Date(`${reportFromDate.value}T00:00:00`)) : null;
    to = reportToDate.value ? endOfDay(new Date(`${reportToDate.value}T00:00:00`)) : null;
    label = "Custom Range";
  }
...
function selectedReportTypeLabel() {
  ...
  return labels[reportType.value] || "All Records";
}

function setReportDatesForPeriod() {
  const period = reportPeriod.value;
  const range = getReportRange();

  if (period !== "custom" && period !== "all") {
    reportFromDate.value = toInputDate(range.from);
    reportToDate.value = toInputDate(range.to);
  }
  ...
  reportFromDate.disabled = period === "all";
  reportToDate.disabled = period === "all";
  reportRangeBadge.textContent = `${selectedReportTypeLabel()} • ${range.label}`;
}
...
function recordDate(record, type) {
  if (type === "memo") return parseRecordDate(record.date) || parseRecordDate(record.createdAt) || parseRecordDate(record.updatedAt);
  return parseRecordDate(record.createdAt) || parseRecordDate(record.updatedAt) || parseRecordDate(record.resolvedAt);
}

function isRecordInRange(record, type) {
  const range = getReportRange();
...
function sortByRecordDate(a, b, type) {
  const aDate = recordDate(a, type) || new Date(0);
  const bDate = recordDate(b, type) || new Date(0);
  return bDate - aDate;
}

function getSystemReportData() {
  const type = reportType.value;
  const includeIncidents = type === "all" || type === "incidents";
  const includeMemos = type === "all" || type === "memos";
  const includeInventory = type === "all" || type === "inventory";

  const reportIncidents = includeIncidents
    ? [...incidents].filter((item) => isRecordInRange(item, "incident")).sort((a, b) => sortByRecordDate(a, b, "incident"))
    : [];

  const reportMemos = includeMemos
    ? [...memos].filter((item) => isRecordInRange(item, "memo")).sort((a, b) => sortByRecordDate(a, b, "memo"))
    : [];

  const reportInventory = includeInventory
    ? [...inventory].filter((item) => isRecordInRange(item, "inventory")).sort((a, b) => sortByRecordDate(a, b, "inventory"))
    : [];

  return { incidents: reportIncidents, memos: reportMemos, inventory: reportInventory };
}

function reportDateText() {
  const range = getReportRange();
  if (range.period === "all") return "All records";
  const fromText = range.from ? formatShortDate(toInputDate(range.from)) : "Beginning";
  const toText = range.to ? formatShortDate(toInputDate(range.to)) : "Today";
  return `${fromText} to ${toText}`;
}

function totalReportRecords(data) {
  return data.incidents.length + data.memos.length + data.inventory.length;
}

function renderIncidentReportSection(rows) {
  if (!rows.length) return "";

  return `
    <section class="report-section-block">
      <h4>Incidents (${rows.length})</h4>
        ...
        <table class="report-table">
          <thead>
            <tr>
              ...
              <th>Resolution / Work Done</th>
              <th>Component / Item Used</th>
              <th>Time Spent</th>
              <th>Updated</th>
            </tr>
          </thead>
          <tbody>
            ${rows.map((incident) => `
              <tr>
                <td><strong>${escapeHtml(incident.id)}</strong></td>
                <td>${formatDate(incident.createdAt)}</td>
                <td>${escapeHtml(incident.requester)}</td>
                <td>${escapeHtml(incident.department)}</td>
                <td>${escapeHtml(incident.location)}</td>
                <td>${escapeHtml(incident.category)}</td>
                <td>${escapeHtml(incident.technician || "-")}</td>
                <td>${escapeHtml(incident.priority)}</td>
                <td>${escapeHtml(incident.status)}</td>
                <td>${escapeHtml(incident.description)}</td>
                <td>${escapeHtml(incident.resolution || "-")}</td>
                <td>${escapeHtml(incident.componentChanged || "-")}</td>
                <td>${escapeHtml(incident.timeSpent || "-")}</td>
        ...
        ${rows.map((incident) => `
          <article class="report-detail-card">
            <div class="ticket-main">
              <div>
                <strong>${escapeHtml(incident.id)} — ${escapeHtml(incident.category)}</strong>
                <p>${escapeHtml(incident.requester)} • ${escapeHtml(incident.department)} • ${escapeHtml(incident.location)}</p>
              </div>
              <div class="badges">
                <span class="badge status-${className(incident.status)}">${escapeHtml(incident.status)}</span>
                <span class="badge priority-${className(incident.priority)}">${escapeHtml(incident.priority)}</span>
            ...
            <p><b>Technician:</b> ${escapeHtml(incident.technician || "-")} &nbsp; <b>Time Spent:</b> ${escapeHtml(incident.timeSpent || "-")}</p>
            <p><b>Issue:</b> ${escapeHtml(incident.description)}</p>
            <p><b>Resolution:</b> ${escapeHtml(incident.resolution || "-")}</p>
            <p><b>Component / Item Used:</b> ${escapeHtml(incident.componentChanged || "-")}</p>
          </article>
        `).join("")}
      </div>
    </section>
...
function memoItemsText(items = []) {
  if (!items.length) return "-";
  return items.map((item) => `${item.name || "Item"} × ${item.quantity || 0} @ ${currency(item.unitCost || 0)}`).join("; ");
}

function renderMemoReportSection(rows) {
  if (!rows.length) return "";

  return `
    <section class="report-section-block">
      <h4>Memos (${rows.length})</h4>
      <div class="table-wrap report-table-wrap">
        <table class="report-table report-table-medium">
          <thead>
            <tr>
              <th>Memo ID</th>
              <th>Date</th>
          ...
          <tbody>
            ${rows.map((memo) => `
              <tr>
                <td><strong>${escapeHtml(memo.id)}</strong></td>
                <td>${formatShortDate(memo.date)}</td>
                <td>${escapeHtml(memo.to || "-")}</td>
                <td>${escapeHtml(memo.from || "-")}</td>
                <td>${escapeHtml(memo.subject || "-")}</td>
                <td>${escapeHtml(memo.purpose || "-")}</td>
                <td>${escapeHtml(memoItemsText(memo.items))}</td>
                <td>${currency(memoTotal(memo.items))}</td>
                <td>${escapeHtml(memo.createdBy || "-")}</td>
      ...
      <div class="report-detail-list">
        ${rows.map((memo) => `
          <article class="report-detail-card">
            <div class="ticket-main">
              <div>
                <strong>${escapeHtml(memo.id)} — ${escapeHtml(memo.subject)}</strong>
                <p>${formatShortDate(memo.date)} • To: ${escapeHtml(memo.to || "-")} • From: ${escapeHtml(memo.from || "-")}</p>
              </div>
              <span class="badge stock-ok">${currency(memoTotal(memo.items))}</span>
            </div>
            <p><b>Purpose:</b> ${escapeHtml(memo.purpose || "-")}</p>
            <p><b>Items:</b> ${escapeHtml(memoItemsText(memo.items))}</p>
          </article>
        `).join("")}
      </div>
    </section>
...
function renderInventoryReportSection(rows) {
  if (!rows.length) return "";

  return `
    <section class="report-section-block">
      <h4>Inventory (${rows.length})</h4>
      <div class="table-wrap report-table-wrap">
        <table class="report-table report-table-medium">
          <thead>
            <tr>
          ...
          <tbody>
            ${rows.map((item) => `
              <tr>
                ...
                <td>${item.quantity} ${isLowStock(item) ? "LOW" : "OK"}</td>
                <td>${item.reorderLevel}</td>
          ...
          <article class="report-detail-card">
            <div class="ticket-main">
              <div>
                ...
                <p>${escapeHtml(item.sku || item.id)} • ${escapeHtml(item.location || "No location")}</p>
              </div>
              <span class="badge ${isLowStock(item) ? "low-stock" : "stock-ok"}">${isLowStock(item) ? "Low Stock" : "In Stock"}</span>
            </div>
            <p><b>Quantity:</b> ${item.quantity} &nbsp; <b>Reorder Level:</b> ${item.reorderLevel} &nbsp; <b>Value:</b> ${currency(item.quantity * item.unitCost)}</p>
            <p><b>Supplier:</b> ${escapeHtml(item.supplier || "-")}</p>
            <p><b>Notes:</b> ${escapeHtml(item.notes || "-")}</p>
          </article>
        `).join("")}
      </div>
    </section>
...
function renderIncidentReport() {
  if (!systemReportOutput) return;

  const data = getSystemReportData();
  const total = totalReportRecords(data);

  reportTotalCount.textContent = total;
  reportIncidentCount.textContent = data.incidents.length;
  reportMemoCount.textContent = data.memos.length;
  reportInventoryCount.textContent = data.inventory.length;
  reportRangeBadge.textContent = `${selectedReportTypeLabel()} • ${getReportRange().label}`;

  if (!total) {
    systemReportOutput.innerHTML = `<p class="empty">No ${escapeHtml(selectedReportTypeLabel().toLowerCase())} found for ${escapeHtml(reportDateText())}.</p>`;
    return;
  }

  systemReportOutput.innerHTML = `
    <div class="report-document">
      <div class="report-title-row">
        <div>
          <p class="eyebrow">The HelpDesk+ System Report</p>
          <h3>${escapeHtml(selectedReportTypeLabel())} • ${escapeHtml(getReportRange().label)}</h3>
          <p>${escapeHtml(reportDateText())}</p>
        </div>
      ...
      ${renderIncidentReportSection(data.incidents)}
      ${renderMemoReportSection(data.memos)}
      ${renderInventoryReportSection(data.inventory)}
    </div>
  `;
}

async function loadIncidentReportFromBackend() {
  if (backendEnabled()) {
    await syncFromBackend(false);
  }
  renderIncidentReport();
  showToast(`Loaded ${totalReportRecords(getSystemReportData())} record(s) for ${selectedReportTypeLabel()} • ${getReportRange().label}.`);
}

function getSystemReportCsvRows() {
  const data = getSystemReportData();
  const rows = [];
  ...
  data.memos.forEach((memo) => {
    rows.push([
      ...
      "",
      currency(memoTotal(memo.items)),
      memo.purpose,
      memoItemsText(memo.items),
      "",
      "",
      memo.updatedAt,
      memo.updatedBy
  ...
  data.inventory.forEach((item) => {
    rows.push([
      ...
      item.notes,
      `${item.movements.length} movement(s)`,
      item.sku,
      "",
      item.updatedAt,
      item.updatedBy
...
function exportIncidentReportCsv() {
  const rows = getSystemReportCsvRows();
  if (!rows.length) {
    showToast("No records found for this report period.");
    return;
  }
  ...
  const headers = [
    ...
    "Main Name / Subject",
    "Category",
    "Department / Location / To-From",
    "Technician / Supplier / Created By",
    "Status / Quantity / Item Count",
    "Priority / Reorder Level",
    "Amount / Stock Value",
    "Description / Purpose / Notes",
    "Resolution / Items / Movements",
    "Component / SKU",
    "Time Spent",
    "Updated At",
    "Updated By"
  ...
  const safeType = selectedReportTypeLabel().toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/(^-|-$)/g, "") || "all";
  const safeLabel = getReportRange().label.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/(^-|-$)/g, "") || "all";
  downloadFile(`system-report-${safeType}-${safeLabel}-${todayISO()}.csv`, csv, "text/csv;charset=utf-8");
  showToast("System report CSV downloaded.");
}

function printIncidentReport() {
  renderIncidentReport();
  if (!totalReportRecords(getSystemReportData())) {
    showToast("No records found for this report period.");
    return;
  }
...
[reportFromDate, reportToDate].forEach((input) => {
...
generateReportBtn.addEventListener("click", async () => {
  try {
    await loadIncidentReportFromBackend();
  } catch (error) {
    showToast(error.message);
  }
});

exportSystemReportBtn.addEventListener("click", exportIncidentReportCsv);
printSystemReportBtn.addEventListener("click", printIncidentReport);

if (quickReportsButton) {
