...
1. Open `service-worker.js` and bump the version number — change
   `helpdesk-plus-v3.1.0` to `v3.1.1`, then `v3.1.2`, and so on.
2. Commit the changes.

Both the cache number above and the small version shown at the bottom of every
page read `v3.1.0` today. Keeping them the same is a choice, not a rule — but
...
> **Your web app URL does not change** when you redeploy as a new version.

### Step 3b — Tell the website where the server is

The website only asks for passwords when it knows your Google server address.
`app.js` ships with that address **empty**, which means *this device only, no
password* mode. Fill it in once, for every device:

1. In Apps Script, **Deploy** → **Manage deployments**, and copy the **Web app
   URL** (it ends in `/exec`).
2. Open `app.js`, search for `backendUrl`, and paste it between the quotes:
   `backendUrl: "https://script.google.com/macros/s/AKfy.../exec",`
3. Bump the version in `service-worker.js` (see "Making future changes"), then
   upload both files.

(You can instead paste the URL under **Settings → Advanced** inside the app,
but that is remembered only by that one browser.)

**How to tell which mode you are in:** if the login screen has **no password
box** and the button says *Continue*, the site is in this-device-only mode.
That is not a fault — it just means the URL above has not been set.

### Step 4 — Everyone creates their own account

There are **no default accounts any more.** The old fixed pair — Julius with
`julius123`, David with `david123` — is gone, and so is the code that created
them. Visit your site and you will see a login screen with a **"New here?
Create an account"** link. Each technician:

1. Opens the site.
2. Taps **Create an account**.
3. Types their name and chooses their own password (10+ characters).
4. They are logged straight in. Nobody else ever sees or sets that password.

There is nothing for you to hand out, publish, or remember on their behalf.

> **If you are upgrading from an older version:** open your Google Sheet's
> **Users** tab and delete every row except the header row, **before** you
> run `setup()` in Step 2. If old rows are left in place, those names stay
> taken and nobody can sign up as "Julius" or "David" again — which is fine
> if the same people are continuing, awkward if you wanted a clean start.

> **Names are how people show up on tickets and memos.** Ask everyone to
> sign up with the same name they want to see in the "Technician" and
> "Assigned to" fields elsewhere in the app.

> **One thing signup does NOT do:** it does not add the person to the
> drop-down lists used to *assign* tickets (the `technician` field on an
> incident, the workload chart on the dashboard). Those still come from a
> plain list in the code — see "Add or remove a technician" in Part 6. After
> someone signs up, add their name there too if you want to be able to
...
**Accounts** (being able to log in) need no editing at all — anyone creates
their own via **Create an account** on the login screen.

**Assignability** (showing up in the "Technician" drop-down on a ticket, and
in the dashboard's workload chart) is still a plain list, because those
drop-downs need to work even before someone has signed up. Open **`app.js`**,
search for **`SECTION 1 of 13`**, and edit this line:

```js
technicians: ["Julius", "David"]
```

Add names in quotes, separated by commas: `["Julius", "David", "Ama"]`. This
does not create a login — it only fills those two drop-downs. Spell each name
exactly as that person used when they signed up.
...
| Problem | Fixed |
|---|---|
| **Saving a server URL in Settings left the password box hidden**, so you were asked for a password with nowhere to type it | The login screen re-checks its mode every time it appears |
| **No way to see what you typed** | A **Show / Hide** button on every password box, and phone keyboards no longer auto-capitalise or auto-correct passwords |
| **Expired sessions were never noticed.** The app kept showing the dashboard while every save failed | An expired login now returns you to the login screen. Unsent changes are kept and send after you log in again |
| **Offline support often never installed** (it waited for a page event that had already happened) | It now installs whether or not the page has finished loading |
| **Connecting to Google Sheets silently erased records saved on the device** | The app now asks first and points you to **Back up** |
| README error message pointed at a step that does not exist | Corrected |

### Fixed in version 3.2.0

| Problem | Fixed |
|---|---|
| **Only two fixed accounts could ever exist** (`Julius` / `David`), with starter passwords published in the old JavaScript | Replaced entirely with self-serve signup: anyone can create their own account with their own password from the login screen. `Code.gs` is a full rewrite — see Step 4 |
| Old default passwords (`julius123` / `david123`) no longer exist anywhere, in code or in docs | — |
...
**Forgot your password / the password will not work.**
Passwords cannot be looked up or emailed — only a scrambled hash is stored,
and a hash cannot be turned back into the password. There is also no "reset
my password" flow, because there is no admin account to grant one. To get
back in:

1. Use the **Show** button on the login screen and retype it slowly. Check
   your name is spelled exactly as you signed up with, and wait 15 minutes if
   you have had several wrong tries in a row.
2. If that genuinely does not work, open your Google Sheet's **Users** tab,
   find your row by name, and delete just that row (leave everyone else's
   alone). Then go back to the site and **sign up again** with the same name
   and a new password. You will not lose any tickets, memos or inventory —
   those live in separate sheets that this does not touch.
3. If you cannot get into the Google Sheet either, ask whoever owns it
   (checked via **Extensions → Apps Script**, or who shared it with you) to
   do step 2 for you.
