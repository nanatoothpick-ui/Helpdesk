...
Passwords cannot be looked up or emailed. Only a scrambled hash is stored, and
a hash cannot be turned back into the password. To get back in:

1. Use the **Show** button on the login screen and retype it slowly. Check the
   right name is selected, and wait 15 minutes if you have had several wrong tries.
2. If the other technician can still log in, that is the easiest route. They are
   no better placed to *read* your password, but once you are back in you can
   choose a new one under **Settings → Change your password**.
3. If nobody can log in, reset the accounts from the Google Sheet. Open the
   **Users** tab and delete the account rows (leave the header row). Then in
   Apps Script choose **`setup`** in the drop-down and press **Run**. With an
   empty Users tab the setup recreates the two starter accounts
   (`julius123` and `david123`). Log in with one and change it at once.
   *This wipes every technician's password, so tell the team first.*
