...
- Weekly, monthly, custom, and all-time incident reports
...
- PWA files for mobile app-style installation
...
The reports module allows the team to load complete records for incidents, memos, inventory, or the whole system.
...
- Full memo records and item totals
- Full inventory/stock records
- Issue descriptions
- Resolution/work done
- Components or items used
- Memo purpose and requested items
- Stock quantity, reorder level, and value
...
| `manifest.json` | PWA/app install settings for phones and desktop browsers |
| `service-worker.js` | Basic offline/app-shell caching for installable app behavior |
...
| `assets/icons/` | App icons for Android, iPhone, browser favicon, and PWA install |
...
The project now includes PWA/app install files:

- `manifest.json`
- `service-worker.js`
- app icons in `assets/icons/`

After deploying to GitHub Pages, open the site on your phone.
...
Note: GitHub Pages uses HTTPS, which is required for PWA installation.
